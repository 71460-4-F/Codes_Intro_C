...
#define N_CADS 50	// capacidade de cadastros
...
{ // função para gravar
	...
	{
		printf("valor de dados no for escrever arq = %d\n\n", dados.quantidade);
		fprintf(arq, "%d\n", i + 1);
		fprintf(arq, "%s\n", dados.vetor_poli[i].nomeCompleto);
		fprintf(arq, "%s\n", dados.vetor_poli[i].apelido);
		printf("dados no vetor salvar apelido  = %s\n", dados.vetor_poli[0].apelido);
		printf("dados no vetor salvar apelido  = %s\n", dados.vetor_poli[1].apelido);
		printf("dados no vetor salvar apelido  = %s\n", dados.vetor_poli[2].apelido);
		printf("dados no vetor salvar apelido  = %s\n", dados.vetor_poli[3].apelido);
		printf("dados no vetor salvar apelido  = %s\n", dados.vetor_poli[4].apelido);
		printf("dados no vetor salvar apelido  = %s\n", dados.vetor_poli[i].apelido);
...
{ // função add politico
	Politico politico[N_CADS];
	printf("\n\n");
	printf("   Cadastro  %d\n", dados.quantidade);
	printf("   Digite o nome: ");
	scanf(" %[^\n]", politico[dados.quantidade].nomeCompleto);
	printf("   Digite o apelido: ");
	scanf(" %[^\n]", politico[dados.quantidade].apelido);
	printf("   Digite o partido: ");
	scanf(" %[^\n]", politico[dados.quantidade].partido);
	printf("   Digite o cargo: ");
	scanf(" %[^\n]", politico[dados.quantidade].cargo);
	printf("   Digite o valor da propina mensal: ");
	scanf("%lf", &politico[dados.quantidade].propiMensal);
	printf("   Digite a quantidade de vezes: ");
	scanf("%d", &politico[dados.quantidade].vezesRecebeu);
	printf("\n   Cadastro realizado com sucesso!\n");
	dados.vetor_poli[dados.quantidade] = politico[dados.quantidade];
	return dados.vetor_poli[dados.quantidade];
}
...
void relatorio_vet(BaseDados dados)
{
	int i;
	int auxi;
	...
	for (i = 1; i < auxi; i++)
	{
		printf("\n");
		printf("   Cadastro %d\n", i);
		printf("   Nome: %s\n", dados.vetor_poli[0].nomeCompleto);
		printf("   Apelido: %s\n", dados.vetor_poli[1].apelido);
		printf("   Partido: %s\n", dados.vetor_poli[2].partido);
		printf("   Cargo: %s\n", dados.vetor_poli[3].cargo);
		printf("   Propina Recebida: R$ %.2lf\n", dados.vetor_poli[4].propiMensal);
		printf("   Vezes que recebeu: %d\n", dados.vetor_poli[5].vezesRecebeu);
		printf("   Total de Propina Recebida: R$ %.2lf\n", dados.vetor_poli[6].propiMensal * dados.vetor_poli[6].vezesRecebeu);
		printf("\n");
	}
	printf("\n\n\n");
	system("pause");
...
void salvar_dados(BaseDados dados)
{
...
{ // função para chamar as outras funçoes do menu
	...
	do
	{
		...
		printf("   (7) Salvar dados no arquivo.\n");
		printf("   (8) Excluir todos os dados.\n");
